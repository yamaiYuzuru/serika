...
exports.execute = async (client, msg, args) => {
  if (!args[1])
    return msg.reply(
      "You must give me an amount of warns to automatic kick the member.\n0 warns disable this function."
    );
  ...
  if (args[1] === 0 && !guildSettings.wtk)
    return msg.reply("Warn to kick is already disabled.");

  if (args[1] === 0) {
    guildSettings.wtk = false;
